PROJECT.PRT.MAIN = function(app) { //главный экран
    var here = this;
    var current_level = 1;
    var win_level_cnt = 0;
    var run = false;
    ...
    here.rand = 214013;
    here.random = function() { //реализация рандома
        var n1 = 214013;
        var n2 = 2531011;
    ...
    */
    ...
    here.get_person = function(info) { //возвращаем персону
    ...
    function init_pereson_object(obj, place, name) { //инициализация человека
        if (name == "")
            name = " ";
        obj.nm = name;
    ...
    function load_current_level() { //загружаем файл декораций
    ...
    function save() {
        var dat = {
            money: here.money,
            player_byed: here.player_byed,
            player_id: here.player_id,
            select_id: here.select_id,
            name: PROJECT.STR.player,
            current_level: current_level,
            win_level_cnt: win_level_cnt
        };
        PROJECT.STR.save(dat);
    }

    function load_game_progress() { //загружаем прогресс игрока
    ...
    function init_level_continue() { //продолжаем после загрузки имен
        ...
        while (here.blocks.length > 0) { //чистим блоки, фонтаны, препятствия
    ...
    here.begin_race = function() { //собственно реальный старт
        here.pos = {
            x: here.app.camera_3d.position.x,
            y: here.app.camera_3d.position.y,
            z: here.app.camera_3d.position.z
        };
    ...
    function start_level() { //нажали на кнопку старт. ждем рекламму и потом показываем появление ботов
        save();

        function start_level_after_ads() { //реклама закончена, пора загружать ботов
            ...
            function on_bots_loading_progress(obj, progress, current_tk, action) {
                if (progress < 1) {
                    ...
                    if (i != here.cur_index) { //рисуем появление ботов
                        here.cur_index = i;
    ...
    here.show_skin = function() { //показываем скин
        ...
        here.msg.btn_r.avk.on_click = function() { //обработчик нажатия кнопки 
            here.select_id++;
        ...
        here.msg.btn_l.avk.on_click = function() { //обработчик нажатия кнопки 
            here.select_id--;
            if (here.select_id < 0)
                here.select_id = here.select.length - 1;
            refresh_gui();
        }
        here.msg.btn_back.avk.on_click = function() { //возврат
            here.app.hide_msg();
            here.msg = null;
            ...
            function on_finish_hint(obj, action, manual_stop) { //цикл показа хинтов
                if (manual_stop) {
                    here.hint_action = null;
                    return;
                }
    ...
    function add_logo(wnd) { //добавляем логотип к окну
        if (!wnd.lg) {
    ...
    here.start_game = function(back) { //начало игры-прелоадер закрыли
        ...
        if (here.msg) { //закрывем вин/луз экран если таковые есть
            here.app.hide_msg();
            here.msg = null;
        ...
        here.select_scene.visible = false;
        if (here.level_objects) { //декор уже загружен
            here.msg.btn_replay.visible = here.msg.btn_skin.visible = true;
            here.msg.wait.visible = false;
        } else {
        ...
        here.msg.btn_replay.avk.on_click = function() { //обработчик нажатия кнопки 
            HideAds();
            here.hint_action.stop();
            here.app.hide_msg();
            here.msg = null;
    ...
    here.move = function(body, tk) {
        ...
        } else if (body.condition == "up_new") { //слетели с горки
        ...
        } else if (body.condition == "up") { //слетели с горки
            body.dx += tk / 1000 * CAMERA_SPEED;
            if (body.dx > 1)
                body.dx = 1;
        ...
        } else if (body.condition == "up_up") { //поднимаемся из воды
            body.dx += tk / 1000 / 1;
            if (body.dx > 1) {
                body.dx = 1;
            }
        ...
        } else if (body.condition == "up_swim") { //плаваем
        ...
        } else if (body.condition == "out") { //улетели
    ...
    function on_fail() {
        ...
        function on_real_fail() {
            ...
            here.msg.btn_replay.avk.on_click = function() {
                PROJECT.STR.on_continue_level(here.start_game)
            };
    ...
    function add_level(wnd) { //добавляем логотип к окну
        if (wnd.cur_lev) {
            wnd.cur_lev.destroy();
            wnd.cur_lev = null;
        }
        ...
        if (current_level + 1 <= PROJECT.STR.max_level()) { //есть уровень
    ...
    function do_ai(tk, min_y) { //типа искусственный интеллект
        for (var i = 0; i < here.bots.length; i++) { //управляем только ботами 
            var bot = here.bots[i];
            if (bot != here.player) {
                bot.change_tk -= tk;
                if (bot.change_tk <= 0) {
                ...
                if ((here.player.condition == "gorka") && (bot.condition == "gorka")) {
                    if (here.player.prg > min_y + 4) { //тормозим ботов
                    ...
                    if (here.player.prg + 5 < min_y) { //ускоряем ботов
                        var ds = 15;
                        bot.multi_speed = bot.ms + ds;
                    } else if (here.player.prg - PROJECT.STR.DIFFICULT < min_y) { //ускоряем ботов немного
    ...
    function update(tk) {
        ...
        if (run) {
            ...
            for (var i = 0; i < here.bots.length; i++) { //апдейтим положение и прогресс, проверяем бонус
                var bot = here.bots[i];
                here.move(bot, tk);
                ...
                if (bot.condition == "gorka") {
                    ...
                    if (bot != here.player) { //вычисляем только для ботов
                        if (max_y < bot.prg)
                            max_y = bot.prg;
                        if (min_y > bot.prg)
                            min_y = bot.prg;
            ...
            if (here.current_finished == 0) { //показываем корону если еще не финишироваля
                for (var i = 0; i < here.bots.length; i++) {
                    var bot = here.bots[i];
    ...
    here.load_level = function(name, on_back) { //загрузка obj файла
        if (here.level_objects) {
